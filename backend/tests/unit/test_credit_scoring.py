...
class TestCreditScoreComputation:
    ...
    def test_zero_revenue_gives_zero_credit_line(self):
        result = compute_credit_score(
            _make_input(revenue_30d=Decimal("0"), revenue_60d=Decimal("0"), revenue_90d=Decimal("0"))
        )
    ...
    def test_minimum_revenue_threshold_boundary(self):
        ...
        result = compute_credit_score(
            _make_input(revenue_30d=MIN_REVENUE_30D)
        )
        assert result.credit_line_amount > Decimal(0)

    def test_higher_revenue_gives_higher_score(self):
        low = compute_credit_score(
            _make_input(revenue_30d=Decimal("500"))
        )
        high = compute_credit_score(
            _make_input(revenue_30d=Decimal("10000"))
        )
        assert high.credit_score >= low.credit_score

    def test_higher_reputation_gives_higher_score(self):
        low_rep = compute_credit_score(
            _make_input(reputation_score=200)
        )
        high_rep = compute_credit_score(
            _make_input(reputation_score=900)
        )
    ...
    def test_code_verified_gives_higher_score(self):
        unverified = compute_credit_score(
            _make_input(code_verified=False)
        )
        verified = compute_credit_score(
            _make_input(code_verified=True)
        )
        assert verified.credit_score >= unverified.credit_score

    def test_higher_consistency_gives_higher_score(self):
        low = compute_credit_score(
            _make_input(consistency_score=Decimal("10"))
        )
        high = compute_credit_score(
            _make_input(consistency_score=Decimal("95"))
        )
        assert high.credit_score >= low.credit_score
...
class TestCreditLineAmount:
    ...
    def test_credit_line_scales_with_revenue(self):
        low = compute_credit_score(
            _make_input(revenue_30d=Decimal("1000"))
        )
        high = compute_credit_score(
            _make_input(revenue_30d=Decimal("10000"))
        )
...
class TestInterestRate:
    ...
    def test_better_score_lower_rate(self):
        ...
        poor = compute_credit_score(_make_input(
            revenue_30d=Decimal("200"),
            consistency_score=Decimal("20"),
            reputation_score=150,
            code_verified=False,
        ))
        good = compute_credit_score(_make_input(
